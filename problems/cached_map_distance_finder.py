    ...
    def get_map_cost_between(self, src_junction: Junction, tgt_junction: Junction) -> Optional[Cost]:
        ...
        if not self._is_in_cache((src_junction.index, tgt_junction.index)):
            ...
            search_result = self.map_problem_solver.solve_problem(
                                  MapProblem(self.streets_map, src_junction.index, tgt_junction.index))
            self._insert_to_cache((src_junction.index, tgt_junction.index),
                                  search_result.solution_cost)
        if not search_result.is_solution_found:
            assert self._get_from_cache((src_junction.index, tgt_junction.index)) is None
        return self._get_from_cache((src_junction.index, tgt_junction.index))
